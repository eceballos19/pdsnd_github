...
def filter_data_time_period(raw_city_data):
...
def statistics():
    ...
    raw_data, city_name = get_data_city()

    # CLean the data for empty values

    # Filter by time period (month, day, none)
    filtered_data, filter_method = filter_data_time_period(raw_data)
    ...
    while True:
        ...
        if restart.lower() in ('yes', 'no'):
            if restart.lower() == 'yes':
                ...
                print('Thanks for using this application. Hope you enjoyed your session.')
            break
        print('Enter a valid input in the options')
